...
function quitarDecimalesCart() {
  if (location.hash == '#/cart') {
    setTimeout(() => {
      let itemsCarrito = document.querySelectorAll(".v-custom-quantity-price__best");
      itemsCarrito.forEach((itemDecimal) => {
        if (itemDecimal.innerText.includes(",")) {
          quitarDecimales(itemDecimal);
        }
      });
      let itemsCarritoNew = document.querySelectorAll(".new-product-price");
      itemsCarritoNew.forEach((itemDecimalNew) => {
        if (itemDecimalNew.innerText.includes(",")) {
          quitarDecimales(itemDecimalNew);
        }
      });
      ...
      if ($("a.boton-volver-home").length == 0) {
  ...
  if (location.hash == '#/email') {
    setTimeout(() => {
      ...
      if(document.querySelectorAll("ul.cart-items li.hproduct.item .description strong.price-subtotal") !== null){
        let subtotalesCol = document.querySelectorAll("ul.cart-items li.hproduct.item .description strong.price-subtotal")
        subtotalesCol.forEach(subtotalItem => {
          subtotalItem.classList.remove("hide")
          quitarDecimales(subtotalItem)
        })
      }
      if ($("a.boton-volver-home").length == 0) {
        $(".transactions-container.row-fluid").append("<a class='boton-volver-home' href='/'><p><</p> Seguir comprando</a>");
  ...
  if (location.hash == '#/payment') {
    setTimeout(() => {
      ...
      //Mostrar Subtotal en cada item de la derecha
      if(document.querySelectorAll("ul.cart-items li.hproduct.item .description strong.price-subtotal") !== null){
        let subtotalesCol = document.querySelectorAll("ul.cart-items li.hproduct.item .description strong.price-subtotal")
        subtotalesCol.forEach(subtotalItem => {
          subtotalItem.classList.remove("hide")
    ...
    enchancementTotalPrice(e) {
      if (this.quantityPriceCart) try {
        $.each(e.items, (function (o) {
          ...
          a.find("td.product-price").find(".vqc-ldelem").remove(), a.find("td.product-price").addClass("v-custom-quantity-price-active").prepend(r).append(`<div class="v-custom-quantity-price vqc-ldelem"><span class="v-custom-quantity-price__best">${n}</span></div>`), a.find("td.product-price").find("> .best-price").wrap('<div class="v-custom-quantity-price__list--selling"></div>'), a.find("td.product-price").find(".v-custom-quantity-price__list--selling").append(`<span class="vqc-ldelem"> ${this.lang?this.lang.eachLabel:"unid"}</span>`)
        }))
      } catch (e) {}
    }
    condensedTaxes(e) {
...
function shippingDay() {
  ...
  if ($(".shp-summary-package-time span").length && !$(".shp-summary-package-time span").hasClass("renew")) {
    let hastaEnvio = document.querySelectorAll(".shp-summary-package-time span")
    hastaEnvio.forEach(hasta => {
      let text1 = hasta.innerText.replace("En hasta", "Desde");
      hasta.innerText = text1;
      hasta.classList.add("renew")
    });
  }

  if ($(".vtex-omnishipping-1-x-leanShippingTextLabelSingle span").length && !$(".vtex-omnishipping-1-x-leanShippingTextLabelSingle span").hasClass("renew")) {
    let hastaEnvio2 = document.querySelectorAll(".vtex-omnishipping-1-x-leanShippingTextLabelSingle span")
    hastaEnvio2.forEach(hasta2 => {
      let text2 = hasta2.innerText.replace("En hasta", "Desde");
      hasta2.innerText = text2;
      hasta2.classList.add("renew")
    });
  }

  if ($("ul.cart-items li.hproduct.item .description span").length && !$("ul.cart-items li.hproduct.item .description span").hasClass("renew")) {
    let hastaEnvio3 = document.querySelectorAll("ul.cart-items li.hproduct.item .description span")
    hastaEnvio3.forEach(hasta3 => {
      let text3 = hasta3.innerText.replace("Hasta", "Desde");
      hasta3.innerText = text3;
      hasta3.classList.add("renew")
    });
  }
